...
> The format identifier stays `constitution-dag-dump/1` (the project was formerly
> named *constitution-dag*; it is now **My Citizen Atlas**). The string is a stable
> version tag baked into every published dump — it is intentionally NOT renamed, so
> existing dumps and validators keep working.
