...
> **Sibling project:** [Citizen Copilot](https://github.com/shaelsrv/citizen-copilot)
> is the *assistant that reads this map for you* — a personal, memory-backed civic
> analyst (hosted or self-hosted) that turns the week's government activity into a
> sourced briefing and traces each event's chain of responsibility down into this
> atlas. My Citizen Atlas is the map; Citizen Copilot is the guide.

## Quickstart (no database needed)

```bash
git clone https://github.com/shaelsrv/mycitizenatlas
cd mycitizenatlas
...
The repository ships the **current** dump; the full version history (what each
prior release added) is preserved in [CHANGELOG.md](CHANGELOG.md).

| dump | contents |
|---|---|
| `dumps/india-0.12.0` | **current** — the genealogy layer, ~278k internal office posts (DM to peon), 10k+ internal feedback loops, 82 citizen task flows, stable external IDs, and the Universal Role Schema (Core v1) export in `role_spec/` |
| `dumps/_template` | a validates-green fictional starter for building your own country's map (`python newdump.py new <slug>`) |

Every dump directory carries `manifest.json` (version, counts, license) and
`SHA256SUMS`, validates clean under `validate.py --strict`, and is mirrored to
Kaggle for data-science use (see `kaggle/`). Format spec: [SCHEMA.md](SCHEMA.md).
Earlier `india-0.1.0`–`0.11.0` builds are summarized in the changelog; only the
current dump is distributed, so what you download always passes today's validator.
...
The one thing we ask (this is the CC-BY attribution): credit **My Citizen Atlas (constitution-dag),
